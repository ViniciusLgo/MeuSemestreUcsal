    ...
    return (
      <div className="text-center py-16">
        ...
        <p className="text-[#8b949e] text-sm">
          Nenhum resultado para <strong className="text-[#e6edf3]">"{q}"</strong>
          {curso && ` no curso ${curso}`}.
        </p>
        <p className="text-[#6e7681] text-xs mt-2">Tente um termo diferente ou verifique a ortografia.</p>
      </div>
    )
  }
    ...
    <div className="mt-8 space-y-8">
      {hasExact && (
        <p className="text-sm text-[#6e7681]">
          ...
          <strong className="text-[#e6edf3]">"{q}"</strong>
          {curso && ` · ${curso}`}
              ...
              <div className="flex flex-wrap gap-2">
                {fuzzyTeachers.map((t) => (
                  <Link
                    key={t.id}
                    href={`/professor/${t.id}`}
                    className="px-3 py-1.5 bg-[#161b22] border border-[#30363d] rounded-full text-sm font-medium text-[#e6edf3] hover:border-[#3fb950] hover:text-[#3fb950] transition-colors"
                  >
                    {t.name}
                  </Link>
                ))}
              ...
              <div className="flex flex-wrap gap-2">
                {fuzzySubjects.map((s) => (
                  <Link
                    ...
                    className="px-3 py-1.5 bg-[#161b22] border border-[#30363d] rounded-full text-sm font-medium text-[#e6edf3] hover:border-[#3fb950] hover:text-[#3fb950] transition-colors"
                  >
                    <span className="font-mono text-xs text-[#6e7681] mr-1">{s.code}</span>
                    {s.name}
                  </Link>
                ))}
              </div>
      ...
      {hasExact && hasFuzzy && (
        <details className="group">
          <summary className="text-xs text-[#6e7681] cursor-pointer hover:text-[#8b949e] list-none flex items-center gap-1">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {[...fuzzyTeachers, ...fuzzySubjects].map((item: any) => (
              <Link
                ...
                className="px-3 py-1.5 bg-[#161b22] border border-[#30363d] rounded-full text-sm text-[#8b949e] hover:text-[#3fb950] hover:border-[#3fb950] transition-colors"
              >
                {item.name}
              </Link>
            ))}
...
function TeacherSection({ teachers }: { teachers: TeacherResult[]; supabase: any }) {
  return (
    <section>
      <h2 className="text-xs font-bold text-[#6e7681] uppercase tracking-widest mb-3">
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        {teachers.map((t) => (
          <Link
            key={t.id}
            href={`/professor/${t.id}`}
            className="flex items-center gap-4 bg-[#161b22] border border-[#30363d] rounded-2xl px-5 py-4 hover:border-[#3fb950] transition-all"
          >
            ...
            <div className="min-w-0 flex-1">
              <p className="font-semibold text-[#e6edf3] truncate">{t.name}</p>
              <p className="text-xs text-[#6e7681] mt-0.5">Ver perfil e avaliações →</p>
            </div>
          </Link>
        ))}
      </div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {subjects.map((s) => (
          <Link
            key={s.id}
            href={`/disciplina/${s.id}`}
            className="bg-[#161b22] border border-[#30363d] rounded-2xl px-4 py-3.5 hover:border-[#58a6ff] transition-all"
          >
            <div className="flex items-start justify-between gap-2 mb-1">
              <span className="font-mono text-xs text-[#6e7681]">{s.code}</span>
              {s.modality === 'ead' && (
            ...
            <p className="text-sm font-semibold text-[#e6edf3] leading-snug">{s.name}</p>
...
export default async function BuscarPage({ searchParams }: Props) {
  ...
  return (
    <div className="container-page py-12 max-w-3xl mx-auto">
      <h1 className="text-3xl font-bold text-[#e6edf3] mb-2">Buscar</h1>
      <p className="text-[#8b949e] text-sm mb-8">Encontre professores e disciplinas da UCSAL Pituaçu.</p>
      ...
      <Suspense
        fallback={
          <div className="mt-12 text-center text-[#6e7681] text-sm animate-pulse">Buscando...</div>
        }
      >
          ...
          <div className="flex flex-wrap justify-center gap-2 mt-4">
            {['Cálculo', 'POO', 'Estrutura de Dados', 'Banco de Dados', 'Redes', 'TCC'].map((s) => (
              <Link
                ...
                className="text-xs bg-[#161b22] border border-[#30363d] text-[#8b949e] px-3 py-1.5 rounded-full hover:bg-[#21262d] hover:border-[#3fb950] hover:text-[#3fb950] transition-colors"
              >
                {s}
              </Link>
            ))}
